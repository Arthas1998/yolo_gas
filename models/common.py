...
def conv_bn(in_channels, out_channels, kernel_size, stride, padding, groups=1):
    ...
    Returns:
        nn.Sequential: 包含卷积层和批量归一化层的序列模块。
    """
...
class SEBlock(nn.Module):
    """
    Squeeze-and-Excitation (SE) 块，用于通道注意力机制。

    Args:
        input_channels (int): 输入通道数。
        internal_neurons (int): 内部神经元数，通常是输入通道数的一半或四分之一。

    Attributes:
        down (nn.Conv2d): 降维卷积层。
        up (nn.Conv2d): 升维卷积层。
        input_channels (int): 输入通道数。
    """
