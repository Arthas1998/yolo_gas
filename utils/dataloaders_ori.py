...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if nl:
            labels[:, 1:5] = xyxy2xywhn(labels[:, 1:5], w=imglist[0].shape[1], h=imglist[0].shape[0], clip=True, eps=1E-3)
        ...
        if mosaic:
            ...
            return torch.from_numpy(np.stack(imglist_orig, 0)), torch.from_numpy(np.stack(imglist, 0)), labels_out, self.im_files[index], shapes #torch.from_numpy(np.stack(imglist, 0)):tensor(K,H,W)
    ...
    def load_imglist(self, i):
        ...
        for i in selected_numbers:
            ...
            imglist.append(cv2.imread(os.path.join(f.rsplit('/', 1)[0], '{:0>5}.jpg'.format(end_index - i)), cv2.IMREAD_GRAYSCALE))
    ...
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            ...
            # place img in img4
            if i == 0:  # top left
                img4list = [np.full((s[0] * 2, s[1] * 2), 114, dtype=np.uint8) for _ in range(len(imglist))] # base image with 4 tiles
                x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
                x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
            elif i == 1:  # top right
